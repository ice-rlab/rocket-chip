  ...
  def toSuperscalarEventSets: SuperscalarEventSets = new SuperscalarEventSets(eventSets.map { case seq =>
    if (seq.size == 1) {
      ...
      //Seq.tabulate(exe_units.length) ( x => ("uops issued" + x, () => exe_req_fire(x)))
        ...
        //seq.map { case set =>
        //  new EventSet(set.events.map { case e =>
        //    val ctr = freechips.rocketchip.util.WideCounterOverflow(
        //      ctr_width, e._2().asUInt, reset, false.B, counter_ack(j))
        //    ctr(e).overflow & counter_ack(j)
        //  })
        //},
        (a: UInt, b: UInt) => a + b
      )
  ...
  def toSuperscalarEventSets: SuperscalarEventSets = {
    pmuMode match {
      case TopdownPMUMode.NONE =>
        new SuperscalarEventSets(eventSets.map { case set =>
          (
            set,
            (a: UInt, b: UInt) => a + b
          )
      ...
      case TopdownPMUMode.SCALAR_COUNTERS =>
        ...
        //new SuperscalarEventSets(eventSets.map(_.head))
